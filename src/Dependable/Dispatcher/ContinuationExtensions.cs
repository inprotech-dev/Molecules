    ...
    public static class ContinuationExtensions
    {
        public static IEnumerable<Continuation> PendingContinuations(this Continuation item, bool isValid = true)
        {
            ...
            if (item.Type == ContinuationType.Single)
            {
                ...
                if (item.Status == JobStatus.Cancelled || item.Status == JobStatus.CancellationInitiated || item.Status == JobStatus.Cancelling)
                {
            ...
            {
                var children = item.Children.Where(_ => _.Status != JobStatus.Cancelled).SelectMany(a => a.PendingContinuations(isValid)).ToArray();
        ...
        public static IEnumerable<Continuation> PendingCancellations(this Continuation item)
        {
            if (item.IsCancelled())
            {
                ...
                cancelledHandlers = item.Children.Where(c => c.IsCancelled())
